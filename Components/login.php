...
<div class="w-full max-w-sm p-6 m-auto mx-auto bg-white rounded-lg shadow-md dark:bg-gray-800">
    ...
    <form class="mt-6" method="POST">
        <div>
            ...
            <input name="username" type="text" class="block w-full px-4 py-2 mt-2 text-gray-700 bg-white border rounded-lg dark:bg-gray-800 dark:text-gray-300 dark:border-gray-600 focus:border-blue-400 dark:focus:border-blue-300 focus:ring-blue-300 focus:outline-none focus:ring focus:ring-opacity-40" />
        </div>
        ...
        <div class="mt-6">
            <button name="submit" class="w-full px-6 py-2.5 text-sm font-medium tracking-wide text-white capitalize transition-colors duration-300 transform bg-gray-800 rounded-lg hover:bg-gray-700 focus:outline-none focus:ring focus:ring-gray-300 focus:ring-opacity-50">
                Sign In
            </button>
        </div>
    </form>
...
<?php 
    if(isset($_POST['submit'])){
     $username = $_POST['username'];
      ...
      if($username == 'philip' AND $password == 'admin123'){
        // header('Manage/overview.php');
    //   header("Location: overview.php");
    echo '<meta http-equiv="refresh" content="1; URL=../Manage/overview.php" />';
